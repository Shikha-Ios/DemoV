//
//  VitaAlertViewController.swift
//  Vita
//
//  Created by Shemona.Puri on 21/07/17.
...
open class SCLAlertViewResponder {
    let alertview: VitaAlertViewController
    
    // Initialisation and Title/Subtitle/Close functions
    
    public init(alertview: VitaAlertViewController) {
...
// The Main Class
open class VitaAlertViewController: UIViewController {
    ...
    fileprivate var selfReference: VitaAlertViewController?
    ...
    fileprivate func setup() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(VitaAlertViewController.tapped(_:)))
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(VitaAlertViewController.keyboardWillShow(_:)), name:NSNotification.Name.UIKeyboardWillShow, object: nil);
        NotificationCenter.default.addObserver(self, selector: #selector(VitaAlertViewController.keyboardWillHide(_:)), name:NSNotification.Name.UIKeyboardWillHide, object: nil);
    }
    ...
    open func addButton(_ title:String, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, showDurationStatus:Bool=false, action:@escaping ()->Void)->SCLButton {
        ...
        btn.action = action
        btn.addTarget(self, action:#selector(VitaAlertViewController.buttonTapped(_:)), for:.touchUpInside)
        btn.addTarget(self, action:#selector(VitaAlertViewController.buttonTapDown(_:)), for:[.touchDown, .touchDragEnter])
        btn.addTarget(self, action:#selector(VitaAlertViewController.buttonRelease(_:)), for:[.touchUpInside, .touchUpOutside, .touchCancel, .touchDragOutside] )
        return btn
    }
    ...
    open func showTitle(_ title: String, subTitle: String, duration: TimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        ...
        if appearance.showCloseButton {
            _ = addButton(completeText ?? "Done", target:self, selector:#selector(VitaAlertViewController.hideView))
        }
        ...
        for btn in buttons {
            btn.backgroundColor = greenBgColor
        ...
        if duration > 0 {
            ...
            durationTimer = Timer.scheduledTimer(timeInterval: self.duration, target: self, selector: #selector(VitaAlertViewController.hideView), userInfo: nil, repeats: false)
            durationStatusTimer?.invalidate()
            durationStatusTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(VitaAlertViewController.updateDurationStatus), userInfo: nil, repeats: true)
        }
